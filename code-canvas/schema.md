  ...
  "regions": [                        // 故事线区域（着色框）
    {
      ...
      "preview": "这条线沿写路径走：…", // 可选，≤320 字：故事线预告——走什么路径、
                                      // 涉及哪些模块/文件、**核心机制怎么运转**
                                      // （数据结构+判据+循环，一段讲透）、读完能
                                      // 懂什么。步进走到该线第一步时底栏自动亮出；
                                      // 手机卡片流的总览目录也逐线展示
  ...
  "cards": [
    {
      ...
      "about": "写路径的入口：…",       // 可选，≤240 字：卡级综述，卡头下常驻。
                                      // **机制句必写**：不止"干什么"，重点是
                                      // "怎么干"——用什么数据结构/判据/步骤把事
                                      // 干成；再带一句在哪个模块、故事里的位置。
                                      // 纯职责定位句（"负责调度"）不合格。
      ...
      "blocks": [                     // 可选：长函数的逻辑块（行段树，非 AST）
        {
          ...
          "explain": "为什么这么写……",  // 可选：块的展开解释（≤120 字），
                                      // 色条出现「说明」按钮，点击展开
          "detail": {                 // 可选：精读模式侧栏的按需展开层
            "why": "…",               // 设计理由（≤200 字）
            "invariants": ["…"],      // 不变量断言（每条 ≤80 字）
            "failure_modes": "…"      // 失败情况/边界（≤200 字）
          },                          // 旧数据缺省即不显示，完全兼容
          "run": {                    // 可选：块沙盘（色条出「试」按钮）——
                                      // 机制核心的块才配，1-3 个/画布，策展
            "lang": "py",             // 可省，v1 只支持 py
            "harness": "…__INPUT__…", // 自包含确定性脚本，恰一处 __INPUT__
                                      // 占位（被 inputs[].value 文本替换）
            "note": "沙盘蒸馏：…",     // ≤70 字：披露相对原文的简化
            "inputs": [               // 3-5 个有讲究的输入
              { "label": "完全命中",   // ≤16 字（典型/边界/反例…）
                "value": "tokens = [1, 2]", // 替进 __INPUT__ 的代码
                "note": "展示只加引用不搬数据", // ≤50 字：看什么
                "expected": "命中…\n" } // 真跑回填的输出——validate 逐个
                                      // 真跑对照，不符即 ERROR；静态画布
                                      // 显示它，连 hub 时可改输入实跑
            ]
...
在线问答的 agent 可在回答末尾附 `canvas-patch` 补丁块（协议注入在问答
提示词里）。持久操作 add_note / add_wire / add_card / set_layout 由
serve.py 套用：过 validate（--no-exec）才写回并重渲染，元素带
`"qa": "<patch_id>"` 标记（渲染成紫虚线描边/虚线线段，与作者手笔一眼
可辨），问答抽屉出「刷新查看 / 撤销」。`show` 操作（聚焦+高亮行）只在
当前页面执行，不落盘。撤销走 POST /unpatch，按 patch_id 连删并还原被
移动卡的原位。不允许 agent 修改已有卡的代码与文字。
