...
3. **读全文**，识别故事线（region）；**每条故事线必写 `preview` 预告**
   （≤240 字）：这条线走什么路径、涉及哪些模块/文件、核心算法或机制、
   读完能懂什么——渲染器在该线第一步自动亮出，让读者 know what to
   expect。**策展连线**：结构层给了全量 call 线，删到只剩故事在场的；
   data 线手工补——只画故事需要的"值的旅程"（值产生行 → 消费卡片，
   带值名 label）
4. **分层细节**：重点卡 `collapsed:false` + 行级 note；次要卡折叠。
   **重点卡必写 `about` 卡级综述**（2-3 句，≤160 字，卡头下常驻）：这个
   函数干什么、在哪个模块、用什么算法/机制、在整个故事里的位置——读者
   读代码前先读它。折叠的上下文卡可省。
