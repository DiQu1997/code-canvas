...
3. **读全文**，识别故事线（region）；**每条故事线必写 `preview` 预告**
   （≤320 字）：这条线走什么路径、涉及哪些模块/文件，**核心机制怎么
   运转**（用什么数据结构、按什么判据、循环怎么推进——一段讲透，不是
   点名词）、读完能懂什么。渲染器在该线第一步自动亮出，让读者 know
   what to expect。**策展连线**：结构层给了全量 call 线，删到只剩故事在场的；
   data 线手工补——只画故事需要的"值的旅程"（值产生行 → 消费卡片，
   带值名 label）
4. **分层细节**：重点卡 `collapsed:false` + 行级 note；次要卡折叠。
   **重点卡必写 `about` 卡级综述**（2-4 句，≤240 字，卡头下常驻）：
   **重点是"怎么干"不是"干什么"**——用什么数据结构、按什么判据、分几步
   把事干成（例：调度器不是"负责调度"，而是"每步先按 token 预算贪心收
   新请求组 prefill 批，组不出才轮 decode，显存不够就抢占最后进来的"）；
   再带一句在哪个模块、在故事里的位置。纯职责定位句不合格。折叠的
   上下文卡可省。
...
5c. **块沙盘**（读代码之外给读者"上手试"的机会，策展不是义务）：
   为 1-3 个**机制核心**的块配 `block.run`（见 schema.md）——
   - `harness`：自包含、**确定性**的可运行脚本（恰一处 `__INPUT__` 占位）。
     原文逻辑蒸馏保真；被简化的部分（缩小规模、换掉外部依赖）必须写进
     `run.note` 披露（≤70 字）。不许 random/时间/网络——validate 会真跑
   - `inputs`：3-5 个**有讲究**的输入，各带 `label`（≤16 字：典型/边界/
     反例…）与 `note`（≤50 字：这个输入展示什么机制侧面）
   - `expected`：**运行 harness 回填**的真实输出，不许手写想象——
     validate 逐个真跑对照，不符即 ERROR。静态画布靠它离线可看；
     连 hub 时读者可改输入真跑
...
- **更大，或用户就说"给我讲讲这个仓库"** → 先出**研究型预览地图**
  （`meta.mode: "preview"`，完整规格见 `preview-spec.md`，要点）：
  - **研究先行，产 JSON 之前必须完成**：读 README/架构文档；
    `extract.py`/`preview.py` 的产出是**研究材料不是答案**；每个候选组
    抽查 2-3 个核心文件读真实代码；形成整体理解一段写进交付报告
  - 卡片 = `kind:"district"` **逻辑分区**（5-9 个，按概念不按目录：一个
    目录可拆进多组，一组可跨多目录——分区与目录一一对应＝研究没做）。
    `highlights` 里的类/函数名必须核实**真实存在**
  - 线 = `kind:"route"` 故事线路（3-6 条，五色地铁线）；每条一个 step：
    `focus`＝站点序、`caption`＝pitch（为什么值得走）、**`ask`＝交给深潜
    生成器的主题句（≤50 字）**——hub 下渲染成「→ 深潜这条」一键点单
  - 读者点单一条线路 → 按细粒度管线另出深潜画布（一个主题一张）
...
- **大仓库直接细讲**：在十万行仓库上硬出函数级画布 = 随机切片，读者不知道
  自己在整体的哪里。先预览地图，后深潜
- **预览图偷懒成目录清单**：分区照抄文件夹、highlights 没核实 = 没做研究
  的伪地图。逻辑分组是研究结论，不是 ls 的输出
